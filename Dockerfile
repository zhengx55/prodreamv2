...
RUN if [ "$BRANCH_NAME" = "test" ]; then cp .env.test .env.production; fi
RUN if [ "$BRANCH_NAME" = "staging" ]; then cp .env.development .env.production; fi
RUN if [ "$BRANCH_NAME" = "main" ]; then cp .env.production .env.production; fi
