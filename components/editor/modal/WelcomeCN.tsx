...
const WelcomeCN = () => {
    ...
    const ImageCard = ({ src, alt, description }: { src: string, alt: string, description: string }) => (
        <div className='text-center flex-shrink-0 rounded-lg bg-[#F8F9FC] px-[20px] py-[24px]'>
            <Image src={src} alt={alt} className="mx-auto mb-2" width={180} height={158} />
            <p className='text-[#475574] text-center text-sm font-normal leading-[160%] mt-[24px]'>{description}</p>
        </div>
    );

    const Footer = () => (
        <div className='text-center'>
            <Button className='' variant={'default'} onClick={() => {
                setShow(false);
            }}>开启ProDream写作之旅</Button>
        </div>
    );
