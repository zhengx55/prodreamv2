    ...
    <section className='relative flex w-full justify-center px-4 pb-[80px] sm:px-0 sm:py-20'>
      <div className='flex-center w-full flex-col gap-y-10 sm:max-w-[1200px]'>
        <m.div
          ...
          className='flex w-full flex-col justify-evenly gap-y-4 sm:flex-row sm:gap-y-0'
        >
          <m.div className='flex w-full flex-col gap-y-2 sm:w-1/3'>
            <h1 className='text-center font-baskerville text-[28px] font-[400] leading-[32px] sm:text-left sm:text-[48px] sm:leading-[58px]'>
              Frequently Asked <br />{' '}
              <span className='relative inline-block before:absolute before:-inset-1 before:top-[22px] before:z-[-1] before:block before:h-[40%] before:-skew-y-0 before:bg-[#D2DFFF] sm:before:top-[36px]'>
                Question&apos;s
              </span>
            </h1>
          ...
          <m.div className='rounded-[8px] bg-[#F5F6F9] p-[10px] sm:w-2/3 sm:rounded-[36px] sm:p-[15px]'>
            {datalist.map((item, index) => (
              <div
                className='mt-[13px] w-full cursor-pointer rounded-[8px] bg-[#fff] px-[17px] py-[12px] sm:mb-[20px] sm:mt-0 sm:rounded-[24px]  sm:p-[36px]'
                key={index}
                onClick={() => {
                  setSelected(index);
                }}
