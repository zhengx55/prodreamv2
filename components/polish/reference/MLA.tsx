...
const MLAReference: React.FC<IMLAReferenceProps> = ({ citation }) => {
  ...
  const generateJournalReference = (citation: IJournalCitation) => {
    ...
    reference += ` "${article_title}." <em>${journal_title}</em>, vol. ${advanced_info?.volume}, no. ${advanced_info?.issue}, ${publish_date?.day} ${publish_date?.month} ${publish_date?.year}, pp. ${page_info?.start}-${page_info?.end}, https://doi.org/${doi}`;
    return reference;
  };
  ...
  const generateChapterReference = (citation: IChapterCitation) => {
    ...
    reference += ` "${section_title}." <em>${book_title}</em>. ${advanced_info?.edition}nd ed., vol. ${advanced_info?.vol} ${advanced_info?.total_vol}, ${publication_info?.publisher}, ${publication_info?.city}, ${publication_info?.state}, ${publication_info?.publish_year}, pp. ${page_info?.start}-${page_info?.end}. ${advanced_info?.series}.`;
    return reference;
  };
