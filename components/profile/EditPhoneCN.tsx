...
import { sendVerificationCodeByPhoneCN, updatePhoneNumber } from '@/query/api';
import { z } from 'zod';
          ...
          <DialogTitle className='flex-between p-0'>
            <p className='text-center text-xl font-normal leading-6 text-[#4B454D]'>
              {userInfo.phone_number
                ? tProfile('Setting.Change_phone_number')
                : tProfile('Setting.Bind_phone_number')}
            </p>
            <DialogClose>
                ...
                <FormItem className='relative'>
                  <div className='flex gap-x-2'>
                    ...
                    <Button
                      ...
                      type='button'
                      variant={'ghost'}
                      className='flex h-14 w-[134px] items-center justify-center gap-2.5 rounded-md border border-[#8652DB] px-3.5 py-1.5'
                    >
                      <span className='text-sm font-normal leading-[150%] text-[#8652DB]'>
                        {verifyWait
                          ? countdown
                          : tAuth('Schema.Send_Verification_Code')}
                      </span>
                    </Button>
                  </div>
