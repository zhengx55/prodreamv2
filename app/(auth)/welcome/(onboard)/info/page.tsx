...
export default async function Page() {
  const datalist = [
    {name: 'Undergrad', desc: `Bachelor degree, Diplomas or highschool`, src: '/welcome/wel1.png'},
    {name: 'Masters or MBA', desc: `Graduate studying a Masters or MBAprogram`, src: '/welcome/wel2.png'},
    {name: 'PhD & Beyond', desc: `Doctorate student or researcher`, src: '/welcome/wel3.png'}
  ]
  return (
    <AnimatedLayout>
      <div className='relative h-max bg-[#fff] rounded-[20px] w-full px-[100px] py-[100px]'>
      <div className='absolute right-[37px] top-[-100px] flex items-center gap-x-1'>
        ...
        <div className='flex justify-center mt-[98px] gap-x-[40px] w-full'>
          {datalist.map((item,index)=>(
            ...
            >
              <div className='w-[360px] h-[500px] rounded-[22px] border-[#D4D3D8] border-[2px] px-[20px] py-[36px] cursor-pointer' key={item.name}>
                ...
                />
                <Spacer y='37' />
                <p className='text-[#525252] text-[24px] font-[300] text-center w-full'>{item.desc}</p>
              </div>
            </Link>
          ))}
        </div>
